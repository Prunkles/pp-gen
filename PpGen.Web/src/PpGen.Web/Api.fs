...
//                    printfn $"Chunk data at ({cx}, {cy})C received"
        ...
        let create seed size =
            let generator cx cy = async {
                let ws = WebSocket.Create($"ws://10.0.0.1:8085/alg/disq/generate?seed={seed}&size={size}&cx={cx}&cy={cy}")
        ...
        let createInterpolated seed size =
            let generator cx cy = async {
                let ws = WebSocket.Create($"ws://10.0.0.1:8085/alg/disq/generate/interp?seed={seed}&size={size}&cx={cx}&cy={cy}")
                return! receiveFromWebsocket ws
            }
            HeightmapGenerator.create generator
