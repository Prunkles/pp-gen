...
В основном были изучены алгоритмы Diamond-Square и шум Перлина. Удалось воссоздать их с модификациями для бесконечной бесшовной генерации.
...
Для почанковой генерации вместо 4-ех начальных точек берется бесконечное множество таких точек (в позициях кратных $2^n$), что каждый чанк размером $2^n + 1$ имеет 4 угловые начальные точки, которые он делит с соседними чанками.

На **Diamond** шаге на границах берется подходящая точка из соседнего чанка. Эта операция так же требует, чтобы в соседнем чанке была сгенерирована нужная точка. Однако для этого прилегающему чанку не нужно генерировать все точки, а только часть наиболее близкую к центральному.

Таким образом для генерации любого произвольного чанка необходимо как минимум создать 16 начальных точек, используемых в 9 чанках и в правильной последовательности *стянуть* их к центральному **Diamond** и **Square** шагами, сгенерировав целый чанк и часть точек вокруг.
...
Ввиду того, что найденные существующие решения не удовлетворяли нижеупомянутым требованиям, было решено разработать собственное программное обеспечение, которое, в добавок ко всему, также способствует развитию знаний, навыков и умений автора работы.
...
- Удалённые вычисления. Ввиду того, что вычисления, производимые во время генерации, достаточно сложные, необходимо добавить возможность производить эти вычисления не прямо на клиенте, а на удалённом сервере.
...
- Клиентская часть приложения написана на языке программирования F#, с использованием компилятора Fable для перевода исходного кода в JavaScript. Для трёхмерного (и двухмерного) рендеринга используется библиотека Three.js.
- Серверная часть приложения написана на языке программирования F#, на платформе .NET, на сервере ASP.NET Core.
- Указание числовых параметров для генерации алгоритмов.
- Алгоритмы
  - Реализован алгоритм Diamond Square, с возможностью бесконечной бесшовной почанковой генерации. Также реализованно отображение с показом интерполированных (усреднённых) результатов.
  - Алгоритм шум Перлина, математически правильная реализация, без упрощений.
...
- Fable — компилятор языка программирования F# в язык программирования JavaScript, который после можно использовать для веб разработки.
...
Программу можно опробовать по ссылке (адресу) {{указать ссылку}}.

![](https://sun9-35.userapi.com/impg/lk7DsnAs2q6RML98AobRY8gKmwm0GmsBgk-EAQ/ee4HTL1e6pQ.jpg?size=1083x706&quality=96&sign=04427f6856b12e426f626ee2644d1725&type=album)
![](https://sun9-40.userapi.com/impg/EVgEHLsgza5HoUcMiraIxOownFYB_ExEBruk1A/5xXkj-fWztU.jpg?size=1086x707&quality=96&sign=4881bf8966e13bb321f550777987db29&type=album)
![](https://sun9-60.userapi.com/impg/ONcSVLmLb7g4cO4zRgNDpriQ9P1pb4Vg-6vLaw/y4XV7dKPMC8.jpg?size=1083x705&quality=96&sign=f3825d1b9b4817ed1cbb96beb829e688&type=album)
